...
export default function App() {
  ...
  useEffect(() => {
    document.title = [selected?.name, me?.site_name || "Flow"].filter(Boolean).join(" · ")
  ...
  return (
    <div className="min-h-svh">
      <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-background/90 backdrop-blur dark:border-slate-800/80">
        <div className="mx-auto flex max-w-[1440px] items-center gap-3 px-4 py-3 sm:px-6 lg:px-8">
          <button className="group flex min-w-0 items-center gap-2.5 text-left" onClick={() => go(null)}>
            <span className="grid size-8 shrink-0 place-items-center rounded-xl bg-slate-900 text-white shadow-sm dark:bg-white dark:text-slate-900">
              <Activity className="size-4" />
            </span>
            <span className="truncate text-sm font-bold tracking-tight text-slate-900 transition-colors group-hover:text-sky-600 dark:text-slate-100 dark:group-hover:text-sky-400">
              {me.site_name || "Flow Monitor"}
            </span>
          </button>
          <span className="hidden rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-600 sm:inline-flex dark:bg-emerald-950/40 dark:text-emerald-400">
            实时监控
          </span>
          <div className="flex-1" />
          <Button variant="ghost" size="icon" className="rounded-xl" onClick={toggleTheme} title="切换主题">
            {dark ? <Sun /> : <Moon />}
          </Button>
        </div>
      </header>
